...
def ensure_data_source(cur) -> str:
    cur.execute(
        ...
        RETURNING id
        """,
        ("finlab", "FinLab Python SDK", "python-sdk", "https://finlab.finance/docs/getting-started/", "Imported by ai-investment-analyst FinLab loader"),
    )
...
def upsert_symbol(cur, market_id: str, spec: StockSpec) -> str:
    ...
    cur.execute(
        ...
        SET name = EXCLUDED.name, local_name = EXCLUDED.local_name, instrument_type = EXCLUDED.instrument_type,
            exchange = EXCLUDED.exchange, currency_code = EXCLUDED.currency_code, country_code = EXCLUDED.country_code, metadata = EXCLUDED.metadata, updated_at = NOW()
        RETURNING id
        """,
        (market_id, spec.stock_id, spec.stock_id, spec.stock_id, spec.instrument_type, "TWSE/TPEx", "TWD", "TW", json.dumps(metadata, ensure_ascii=False)),
    )
    return _row_id(cur.fetchone())
...
def create_ingestion_run(cur, data_source_id: str, market_id: str, stock_ids: list[str]) -> str:
    cur.execute(
        ...
        RETURNING id
        """,
        (data_source_id, "manual-finlab-load", "price_daily_raw,price_daily_canonical", market_id, "running", json.dumps({"stock_ids": stock_ids}, ensure_ascii=False)),
    )
    return _row_id(cur.fetchone())
...
def store_price_row(cur, *, symbol_id: str, data_source_id: str, ingestion_run_id: str, trading_date, close_price: Decimal | None, volume: int | None) -> None:
    turnover_value = close_price * Decimal(volume) if close_price is not None and volume is not None else None
    raw_payload = {"close_price": str(close_price) if close_price is not None else None, "volume": volume}
...
def load_finlab_price_data(stock_ids: tuple[str, ...] = DEFAULT_STOCK_IDS, lookback_rows: int = DEFAULT_LOOKBACK_ROWS) -> dict[str, Any]:
    ensure_finlab_login()
            ...
            with conn.cursor() as cur:
                for stock_id in stock_ids:
                    ...
                    for trading_date, close_value in stock_close.items():
                        ...
                        store_price_row(cur, symbol_id=symbol_id, data_source_id=data_source_id, ingestion_run_id=ingestion_run_id, trading_date=trading_date.date(), close_price=decimal_or_none(close_value), volume=int_or_none(volume_value))
                    ...
                    summary["processed"].append({"stock_id": stock_id, "rows": len(stock_close), "latest_date": latest_date.isoformat() if latest_date else None, "latest_close": str(latest_close) if latest_close is not None else None})
