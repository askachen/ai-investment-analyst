...
def ensure_data_source(cur) -> str:
    cur.execute(
        ...
        (
            ...
            "rest-api",
            FINMIND_BASE_URL,
            "Imported by ai-investment-analyst FinMind loader",
        ),
    )
...
def upsert_price_daily(cur, symbol_id: str, ingestion_run_id: str, row: dict[str, Any]) -> None:
        ...
        try:
            with conn.cursor() as cur:
                for stock_id in stock_ids:
                    ...
                    for row in rows:
                        upsert_price_daily(cur, symbol_id, ingestion_run_id, row)
